Fix the Android project at /tmp/mhstyle-rush so the failing quality-fix-1 step passes.
...
Recent failure log:
```text
=== QUALITY CHECK: /tmp/mhstyle-rush ===
...
WARN: Only 1 commit(s) — final implementation commit may not exist yet
  ...
  OK: icon.png (230880B, 512x512, rounded opaque canvas, transparent corners)
FAIL: Manifest references .SkeletonApplication but class not found — CRASH
  OK: HomeScreen.kt: 173 lines
